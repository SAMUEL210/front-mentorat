        ...
        .then(function(reponse) {
            for (contrat of reponse.data.contrats) {
                if (contrat.mentorId == sessionStorage.getItem('utilisateur')) {
                    var utId = contrat.mentoreId
                }
                if (contrat.mentoreId == sessionStorage.getItem('utilisateur')) {
                    var utId = contrat.mentorId
                }
                if (contrat.mentorId == sessionStorage.getItem('utilisateur') || contrat.mentoreId == sessionStorage.getItem('utilisateur')) {
                    axios({
                        method: 'get',
                        url: 'https://api-simarone-fusecrum.herokuapp.com/utilisateurs/' + utId,
                        headers: {
                            'x-access-token': sessionStorage.getItem('x-access-token')
                        },
                        responseType: 'json'
                    }).then(function(donneeUtId) {
                        var ut = donneeUtId.data.utilisateur
                        let div = document.createElement('div')
                        let p = document.createElement('p')
                        let form = document.createElement('form')
                        let input = document.createElement('input')
                        ...
                        p.innerHTML = "Avec " + ut.prenom + ' ' + ut.nom + ', ' + ut.poste
                        p.className = 'p-contrats'
                        div.appendChild(p)
                        ...
                        form.appendChild(button)
                        div.appendChild(form)
                        contrats.appendChild(div)

                        div.style.display = 'flex'
                        div.style.justifyContent = 'space-between'
                        div.className = 'div-contrats breadcrumb'
                    });
